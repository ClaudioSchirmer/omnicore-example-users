...
| `upstream_composition.sh` | service-to-service upstream composition — filtered local projection + the composed read view (`gadgets_composed` one-to-one-embeds the projection under `upstreamMirror`, served over `GET /qa/gadgets-composed/:id`) + onUpstreamDelete cascade + failure registry/admin drain | self-managed | qa binary + `microservice.qa.yaml`; needs CDC; ~40s |
