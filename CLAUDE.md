...
**`AppContext` thread-through**: all wrappers (`HandleCommandWithBody{,ID}`, `HandleCommandWithID`, `HandleQueryWith{Params,ID}`) populate `appCtx.SetParent(c.UserContext())` before Dispatch — `AppContext` implements `context.Context`, so request cancellation propagates all the way down to `ViewReader.ReadPage`/`ReadByID` and to any Repository call that takes a context.
...
| `web/user_custom_routes.go` | — | Plain Fiber handlers: `BodyParser → AppContext → ToCommand → Dispatch → switch result` |
...
handler (web/showcase_routes.go)
   └─ fwweb.AppContext(c).SetParent(c.UserContext())
...
3. fwweb.HandleCommandWithBody → var req InsertUserRequest + BodyParser(&req)
...
3.5. appCtx := fwweb.AppContext(c); appCtx.SetParent(c.UserContext())
