...
| GET | `/users.xlsx` | `HandleQueryAsXLSX` (via `MountRaw`, app-root) | same DTO + `view.ExportPlan()` | same handler as `GET /users` | 200 Excel `.xlsx` — identical surface to `/users.csv`, different encoder (bold headers, typed numeric cells, sheet `Users`). Showcases the format-pluggable export: only the encoder differs |
