...
**Consuming unreleased framework surface is a structural dependency, not a "later" detail.** When this service references an `omnicore` symbol that exists only on an unmerged framework branch, a local `go build ./...` still passes — `go.work` overlays the local checkout — but CI resolves `omnicore` from the proxy at the version pinned in `go.mod`, so it fails until the framework is **tagged** (or a pseudo-version is pinned) and `go.mod` is bumped. Flag any such cross-module reference as a release-ordering dependency the moment it is introduced; the example's CI cannot go green before the framework tag exists.
...
│   │   ├── find_users_by_params_request.go    # FindUsersByParamsRequest + ToQuery(criteria) + FindUsersByParamsResponse (json: tags, projected via fwresponses.AutoFromDoc)
│   │   ├── find_user_by_id_request.go         # FindUserByIDRequest + ToQuery() + FindUserByIDResponse (json: tags, projected via fwresponses.AutoFromDoc)
...
**The `users`/`addresses` tables, their columns, and the FK `user_id` are declared explicitly** in `infra/schema.go` via `UserSchema()` / `AddressSchema()` (the mandatory `TableSchema` — `Field("ZipCode","zip_code")` etc.). The domain stays DDD-pure: it never pronounces a table, column, or FK name.
...
- **No `ToFields()`** — `AddressSchema()` maps each persisted field to its column; `ID` is the PK (skipped on the write list, used in WHERE); FK `user_id` is declared via `.FK("user_id")` and injected by the persister (not a struct field).
...
- **`fwinfra.AggregateLoader[*User]`** in **schema-driven auto-scan mode** — no manual scanner. The explicit `UserSchema()` is threaded via `WithSchema`; its declared columns drive the generated SELECT (`SELECT id, name, email, phone FROM users WHERE id=$1 AND deleted_at IS NULL`). Address is NOT registered with a separate call — it comes from the schema's `.Child(AddressSchema())` declaration (table `addresses`, columns, FK `user_id` all explicit).
...
Result: zero scanners — the Repository declares the mapping once via `WithSchema(UserSchema())` and both `UserRepository` and `UserCustomRepository` reuse the same schema. For non-trivial queries, `WithRootScanner`/`WithChildScanner` are still available and coexist with the schema-driven auto-scan by typeName.
...
2. **Layer 2 — programmatic owner-check (domain)**: `domain/user.go` carries runtime-only fields `RequestingPrincipalEmail` + `RequestingPrincipalIsAdmin` (plain exported fields with NO tag — they are NOT declared in `UserSchema()`, so the persister never writes, scans, or audits them); `BuildRules.IfUpdate` checks them when `actionName == "GetArchivable"`. Rule: "the JWT email claim must match the persisted User's email, unless the principal carries `users:admin`" (super-admin bypass via `*:*`). The fields are populated by `application/commands/archive_user.go::ApplyTo(ctx, u)` — the canonical place to translate `ctx.Identity()` into business-named fields. Tolerant of nil Identity (degraded "trust" mode under `auth.mode=disabled`).
...
**Read-side Response (projector) — same rule on both surfaces.** Every GET wrapper takes a mandatory projector `func(map[string]any) R`. Both surfaces pair with `fwresponses.AutoFromDoc[FindXxxResponse]` — the framework's tag-driven default that consumes only `json:"<wire>"` for the outgoing JSON name and keys into the doc by the Go field name (the reader already returned a Go-keyed doc). The manual showcase routes (`GET /showcase/users-custom`, `GET /showcase/users-custom/:email`, `GET /showcase/users-custom/:email/addresses/:addressId`) use the SAME `AutoFromDoc[R]` projector as the canonical — projection is shared infrastructure, not a wrapper detail. The "manual" in manual showcase applies to the orchestration steps (`Bind().Body() → BindPath → QueryParser → Dispatch → RespondWithSuccess / RespondPaged`), not to hand-rolling a projector that does the same thing as `AutoFromDoc`. Declare a consumer `R{}.FromDoc` method only when the projection needs logic AutoFromDoc cannot express (derived fields, conditional shaping, ctx-aware projection). Consumers that genuinely want the raw Mongo doc on the wire pass `fwresponses.RawDoc` instead. The manual surface gets the same allowlist enforcement + sparse-render boot guard as the canonical via `fwweb.NewQueryParser[Req, Resp]` constructed at `MountUsersCustom` time — unknown keys produce the canonical 400 envelope, and a Response that violates `*T + ,omitempty` recursively panics at boot (see "Manual showcase" Read side).

**Why `ZipCode` carries no `view:` tag (there is none).** The Composer writes the physical column name `zip_code` to the Mongo doc, but the `MongoViewReader` translates it back to the Go field name `ZipCode` via `AddressSchema()` (`Field("ZipCode","zip_code")`) **before** projection. So the typed Response carries only `json:"zipCode"` — the outgoing wire name — and the projector reads the doc by the Go field `ZipCode`. `?fields=addresses.zipCode` translates to `{"addresses.zip_code":1}` via the view `TableSchema` (not `PascalToSnake`). A renamed column is handled entirely on the infra side by the schema; the Response never pronounces a physical name and there is no source-key tag.
...
| PATCH | `/users/:id/archive` | `HandleCommandByID` | — | `ArchiveCommandHandler[*User, *ArchiveUserCommand, fwresults.None]` — aggregate-aware (cascades addresses) | 200, no `data` |
| PATCH | `/users/:id/unarchive` | `HandleCommandByID` | — | `UnarchiveCommandHandler[*User, *UnarchiveUserCommand, fwresults.None]` — aggregate-aware (restores archived) | 200, no `data` |
| DELETE | `/users/:id` | `HandleCommandByID` | — | `DeleteCommandHandler[*User, *DeleteUserCommand, fwresults.None]` (hard) | 204, no `data` |
...
| GET | `/users.csv` | `HandleQueryAsCSVSpec` + canonical `openapi.Mount` (app-root path to avoid the `/users/:id` collision) | `FindUsersByParamsRequest` (reused) + `view` (`web.ExportView`) + `d.Export` (`web.ExportDeps`) | `FindByParamsQueryHandler[*FindUserByParamsQuery]` (same handler as `GET /users`) | 200 `text/csv`, hierarchical (root cols at A, addresses at B), labelKey headers per `Accept-Language`, `?fields=` narrows; user pagination ignored, capped at `query.maxExportRows`; `,` delimiter; blank line separates each aggregate block. Filters render in Swagger (RequestType reflected) + 200 documented as a download (`RouteSpec.FileResponse`) |
| GET | `/users.xlsx` | `HandleQueryAsXLSXSpec` + canonical `openapi.Mount` (app-root) | same DTO + `view` + `d.Export` | same handler as `GET /users` | 200 Excel `.xlsx` — identical surface to `/users.csv`, different encoder (bold headers, typed numeric cells, sheet `Users`). Showcases the format-pluggable export: only the encoder differs |
| GET | `/users/:id` | `HandleQueryByID` | `FindUserByIDRequest` + `fwresponses.AutoFromDoc[FindUserByIDResponse]` | `FindByIDQueryHandler[*FindUserByIDQuery]` (Mongo via `d.ViewReader`) | 200 with `FindUserByIDResponse` / 404 |
...
| `web/user_routes.go` | `fwweb.HandleCommandWithBody{,ID}` + `HandleCommandByID` | — |
...
- **GraphQL surface (§16)** — `POST /graphql` carries the identical Layer-1 gate: no bearer → 401 `MissingAuthorizationNotification` (AuthMiddleware, before the gate); `noperm` → `MissingPermissionNotification` in `errors[].extensions` for `users` (read) and `createUser` (write); alice's `users` read resolves (has `users:read`); alice's `deleteUser` → `MissingPermissionNotification` (lacks `users:delete`). **Field-level read access** also rides the GraphQL surface: alice (non-admin) explicitly selecting `node { phone }` → `FieldAccessForbiddenNotification` (the selection set is mapped to `ReadCriteria.Projection`, so the restricted field trips the same active-reference 403 as the REST `?fields=phone`); not selecting it resolves (passive scrub, never leaked). Asserted via a `show_gql_case` twin that reads `errors[0].extensions.notificationKey` (the GraphQL error shape) instead of the REST `messages[]` envelope.
...
- **All six write verbs (parity with `/users/*`)** — `updateUser` (PUT, `MutationWithBodyID`, strict full body, email kept immutable), `patchUser` (PATCH, `MutationWithBodyID`, partial), and the bodyless `archiveUser` / `unarchiveUser` / `deleteUser` (`MutationByID` → `MutationResult{success}`); `deleteUser` also cleans up the created record.
- **Count-only (`totalCount`-only selection)** — `users { totalCount }` with no `edges`/`pageInfo` maps to `ReadCriteria.OnlyTotal` (the GraphQL idiom for REST `?onlyTotal=true`): the reader short-circuits to `CountDocuments` and the envelope carries `totalCount` with no `edges` key. A pagination/sort argument (`first`/`last`/`after`/`before`/`orderBy`) alongside a `totalCount`-only selection is a **conflict** — rejected pre-dispatch with `SchemaViolationNotification` (`semantic: "Schema"`, the offending argument as `field`), parity with REST's `onlyTotalConflicts`.
- **Notifications surface legibly in `errors[].extensions`** — a domain rule (`createUser` with a malformed email → `InvalidEmailNotification`) carries the full triple `semantic: "Validation"` / `notificationKey` / `field`; a stale/foreign keyset cursor on a real (`edges`) read (`users(after: …)` whose context hash does not match) reaches the reader and surfaces as `semantic: "Schema"` / `SchemaViolationNotification` / `field: "cursor"` — NOT the opaque `"internal server error"` / `Internal` (GraphQL has no pre-dispatch cursor check on a real read, so the reader returns the typed `infra.InvalidCursorError`). Note the count-only path above rejects the same stale cursor *earlier*, as a pre-dispatch conflict, when the selection is `totalCount`-only.
- **Relay pagination direction (`first`/`after` forward, `last`/`before` backward)** — `users(last: 1)` returns a well-formed backward connection (it pages from the END of the set; the per-row ordering correctness is covered by the framework's reader integration test). Forward and backward arguments are mutually exclusive: `first`+`last`, `last`+`after`, and an `after`+`before` pair are each rejected pre-dispatch with `SchemaViolationNotification` (`semantic: "Schema"`) — the `after`+`before` case is now a clean 400 here, not the reader's defense-in-depth 500. A non-positive page size (`first: 0`) is rejected the same way, parity with REST rejecting `?limit=` ≤ 0.

Total: ~38 cases. Server-running pattern (no self-lifecycle); needs the CDC pipeline up for the read case.
