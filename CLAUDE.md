...
| GET | `/users.csv` | `HandleQueryAsCSVSpec` + canonical `openapi.Mount` (app-root path to avoid the `/users/:id` collision) | `FindUsersByParamsRequest` (reused) + `view.ExportPlan()` | `FindByParamsQueryHandler[*FindUserByParamsQuery]` (same handler as `GET /users`) | 200 `text/csv`, hierarchical (root cols at A, addresses at B), labelKey headers per `Accept-Language`, `?fields=` narrows; user pagination ignored, capped at `query.maxExportRows`; `,` delimiter; blank line separates each aggregate block. Filters render in Swagger (RequestType reflected) + 200 documented as a download (`RouteSpec.FileResponse`) |
| GET | `/users.xlsx` | `HandleQueryAsXLSXSpec` + canonical `openapi.Mount` (app-root) | same DTO + `view.ExportPlan()` | same handler as `GET /users` | 200 Excel `.xlsx` — identical surface to `/users.csv`, different encoder (bold headers, typed numeric cells, sheet `Users`). Showcases the format-pluggable export: only the encoder differs |
