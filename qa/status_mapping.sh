...
SERVER_BIN="/tmp/omnicore-example-users-qa-status-mapping-${BACKEND:-postgres}"
SERVER_LOG="/tmp/omnicore-example-users-qa-status-mapping-${BACKEND:-postgres}.log"
...
cleanup() {
  ...
  kill_port "${HTTP_PORT:-8080}"; rm -f "$LOW_TIMEOUT_YAML"
