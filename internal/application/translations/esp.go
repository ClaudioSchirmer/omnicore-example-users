package translations

import (
	"github.com/ClaudioSchirmer/omnicore/application/configuration"
	"github.com/ClaudioSchirmer/omnicore/application/translation"
)

type esp struct{}

// ESP returns the Spanish translation module for this service's custom
// notifications. Register alongside translation.CoreES() at startup.
func ESP() translation.Module { return esp{} }

func (esp) Language() configuration.Language { return configuration.LangES }

func (esp) Translations() map[string]string {
	return map[string]string{
		"InvalidEmailNotification":          "Email inválido.",
		"InvalidPhoneNotification":          "Número de teléfono inválido.",
		"InvalidDocumentNotification":       "Documento inválido.",
		"InvalidStateNotification":          "Estado inválido.",
		"InvalidZipCodeNotification":        "Código postal inválido.",
		"InvalidCountryNotification":        "País inválido (use código ISO de 2 letras).",
		"DocumentCannotChangeNotification":  "El documento no puede modificarse después de crear el usuario.",
		"DuplicateAddressNotification":      "Dirección duplicada para este usuario.",
		"ProductCategoryLimitNotification":  "Límite de categorías de producto distintas alcanzado.",
		"NameMaxLengthExceededNotification": "El nombre supera la longitud máxima permitida de {maxLength} caracteres.",
		"User":                              "Usuario",
		// Field labels — see ptbr.go for the per-locale rationale.
		"UserNameField":                  "Nombre",
		"UserEmailField":                 "Correo electrónico",
		"UserPhoneField":                 "Teléfono",
		"UserDocumentField":              "Documento",
		"UserUserNameField":              "Nombre de usuario",
		"UserEmailNotificationField":     "Notificación por correo",
		"UserSmsNotificationField":       "Notificación por SMS",
		"PersonEthnicityField":           "Etnia",
		"UserProfileField":               "Perfil",
		"UserNotificationEmailField":     "Correo de notificación",
		"UserNotificationFrequencyField": "Frecuencia de notificación",
		"AddressLabelField":              "Etiqueta",
		"AddressStreetField":             "Calle",
		"AddressNumberField":             "Número",
		"AddressComplementField":         "Complemento",
		"AddressNeighborhoodField":       "Barrio",
		"AddressCityField":               "Ciudad",
		"AddressStateField":              "Estado",
		"AddressZipCodeField":            "Código postal",
		"AddressCountryField":            "País",
		"AddressTypeField":               "Tipo de dirección",
		// Agregado Employee — etiqueta de contexto, etiquetas de campo y notificaciones.
		"Employee":                                 "Empleado",
		"EmployeeNumberField":                      "Matrícula",
		"EmployeeBankField":                        "Banco",
		"EmployeeBranchField":                      "Sucursal",
		"EmployeeAccountField":                     "Cuenta",
		"EmployeePixField":                         "Clave Pix",
		"DependentNameField":                       "Nombre del dependiente",
		"DependentBirthDateField":                  "Fecha de nacimiento",
		"DependentRelationshipField":               "Parentesco",
		"DependentHealthPlanProviderField":         "Proveedor del plan de salud",
		"DependentHealthPlanCardField":             "Tarjeta del plan de salud",
		"DependentHealthPlanExpiryField":           "Vigencia del plan de salud",
		"DependentHealthPlanTypeField":             "Tipo de plan de salud",
		"JobHistoryJobTitleField":                  "Cargo",
		"JobHistoryDepartmentField":                "Department",
		"JobHistoryHiredAtField":                   "Fecha de contratación",
		"JobHistoryTerminatedAtField":              "Fecha de desvinculación",
		"UnknownRelationshipNotification":          "Parentesco desconocido (use spouse, son, daughter, father, mother u other).",
		"UnknownEthnicityNotification":             "Etnia desconocida.",
		"UnknownAddressTypeNotification":           "Tipo de dirección desconocido.",
		"UnknownUserProfileNotification":           "Perfil de usuario desconocido.",
		"UnknownNotificationFrequencyNotification": "Frecuencia de notificación desconocida.",
		"UnknownHealthPlanTypeNotification":        "Tipo de plan de salud desconocido.",
		"InvalidHealthPlanCardNotification":        "Tarjeta del plan de salud inválida.",
		"TerminationBeforeHireNotification":        "La fecha de desvinculación no puede ser anterior a la contratación.",
	}
}
