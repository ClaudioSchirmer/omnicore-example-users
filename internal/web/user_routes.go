...
func MountUsers(
	...
	fwopenapi.Mount(d.OpenAPIRegistry, users, fiber.MethodPatch, "/:id/archive",
		...
		fwopenapi.Doc{
			...
			Description: "Archive via `deleted_at = NOW()`. Aggregate-aware: the same TX archives every active address. Symmetric inverse of `/unarchive`. Layer-2 ownership applies — the JWT principal's email claim must match the persisted user's email unless they carry `users:admin` (super-admin bypass via `*:*`).",
			Tags:        []string{"Users"},
		},
