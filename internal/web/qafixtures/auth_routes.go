...
//
//	POST /qa/auth/login          {subject, permissions?, audience?} -> access+refresh
...
func loginHandler(d bootstrap.Deps) fiber.Handler {
	return func(c fiber.Ctx) error {
		...
		return fwweb.RespondWithSuccess(c, fiber.StatusOK, tokenPairResponse{
			AccessToken: access.Token, ExpiresAt: access.ExpiresAt, JTI: access.JTI, RefreshToken: refresh.Value,
		})
	}
}
