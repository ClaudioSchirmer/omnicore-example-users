...
// port appdomain.UserRepository, ready to use — reads and writes both run
// on the returned value.
//
// The repository PORT itself is a domain concept (appdomain.UserRepository in
// domain/user_repository.go). This provider is the binding seam: it lives in
...
type ScopedUserRepository interface {
	Scope(ctx *configuration.AppContext, opts ...persistence.WriteOption[*appdomain.User]) appdomain.UserRepository
}
