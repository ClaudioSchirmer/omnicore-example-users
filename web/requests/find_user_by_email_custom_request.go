...
// FindUserByEmailCustomRequest is the wire allowlist for GET
// /showcase/users-custom/:email. Only ?includeArchived=true|false is
...
// auto HandleQueryWithID wrapper enforces on /users/:id.
//
// Email carries `path:"email"`: the manual route chains fwweb.BindPath
// before fwweb.ParseCriteria so the framework reads c.Params("email")
// into req.Email automatically — same mechanism the canonical wrappers
// use internally. ToQuery's signature now matches every other ToQuery in
// the project (single ReadCriteria parameter, no side strings).
type FindUserByEmailCustomRequest struct {
	Email           string `path:"email"`
	IncludeArchived *bool  `query:"includeArchived"`
}

// ToQuery is the web→application boundary — pure body mapping with no ctx.
// Reads req.Email (populated by BindPath from the URL segment) and the
...
// FindUserByEmailCustomResponse is the reduced wire projection of one User
...
//
// Projection runs through fwresponses.AutoFromDoc[FindUserByEmailCustomResponse]
...
type FindUserByEmailCustomResponse struct {
